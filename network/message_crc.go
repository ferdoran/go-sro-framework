...
func GenerateTable() []uint32 {
	baseTable := [256]uint32{
	...
	finalIndex := 0
	finalTable := make([]uint32, 256*256)

	for i := 0; i < 256; i++ {
		var edx = baseTable[i]
		for ecx := uint32(0); ecx < 256; ecx++ {
			eax := ecx >> 1
			if (ecx & 1) != 0 {
				eax ^= edx
			}
...
func NewMessageCRC(seed uint32) MessageCRC {
	newseed := seed << 8
	return MessageCRC{seed: newseed}
}

func (m *MessageCRC) Compute(buffer []byte) byte {
	checksum := uint32(0xFFFFFFFF)

	for x := 0; x < len(buffer); x++ {
		checksum = checksum>>8 ^ table[m.seed+((uint32(buffer[x])^checksum)&0xFF)]
	}
